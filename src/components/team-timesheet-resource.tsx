...
type Timesheet = { id: string; employee_id: string; project_id: string | null; work_date: string; hours: number; notes: string | null; employees: { name: string } | null; projects: { name: string } | null };
...
export function TeamTimesheetResource({ type }: { type: "team" | "timesheets" }) {
  ...
  const visibleEmployees = employees.filter((employee) => [employee.name, employee.job_title, employee.email].filter(Boolean).join(" ").toLowerCase().includes(search.trim().toLowerCase()));
  const visibleTimesheets = timesheets.filter((timesheet) => (!employeeFilter || timesheet.employee_id === employeeFilter) && (!projectFilter || timesheet.project_id === projectFilter));
  const today = new Date();
  const monday = new Date(today); monday.setDate(today.getDate() - ((today.getDay() + 6) % 7));
  const weekStart = monday.toISOString().slice(0, 10);
  const weeklyHours = timesheets.filter((timesheet) => timesheet.work_date >= weekStart).reduce((total, timesheet) => total + Number(timesheet.hours || 0), 0);
  return <AppShell title={isTeam ? "Team" : "Timesheets"} description={isTeam ? "Manage employees, roles and site access." : "Track work hours across every construction site."}>
    ...
    {!isTeam && <><div className="mt-5 grid gap-4 sm:grid-cols-2"><div className="rounded-2xl border border-slate-200 bg-white p-5"><p className="text-sm text-slate-500">Hours this week</p><p className="mt-2 text-3xl font-bold">{weeklyHours}h</p></div><div className="rounded-2xl border border-slate-200 bg-white p-5"><p className="text-sm text-slate-500">Logged entries</p><p className="mt-2 text-3xl font-bold">{timesheets.length}</p></div></div><div className="mt-5 grid gap-3 sm:grid-cols-2"><select value={employeeFilter} onChange={(event) => setEmployeeFilter(event.target.value)} className="rounded-xl border border-slate-300 bg-white p-3 text-sm"><option value="">All employees</option>{employees.map((employee) => <option key={employee.id} value={employee.id}>{employee.name}</option>)}</select><select value={projectFilter} onChange={(event) => setProjectFilter(event.target.value)} className="rounded-xl border border-slate-300 bg-white p-3 text-sm"><option value="">All projects</option>{projects.map((project) => <option key={project.id} value={project.id}>{project.name}</option>)}</select></div></>}
    ...
    {isTeam && <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search employees..." className="mt-6 w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm outline-none focus:border-violet-500" />}
    <section className="mt-6 overflow-hidden rounded-2xl border border-slate-200 bg-white">{isTeam ? visibleEmployees.map(e => <article key={e.id} className="flex items-center justify-between border-b p-5 last:border-0"><div><Link href={`/team/${e.id}`} className="font-semibold text-slate-950 hover:text-violet-700">{e.name}</Link><p className="text-sm text-slate-500">{e.job_title ?? "Employee"}{e.email ? ` · ${e.email}` : ""}</p></div><select aria-label={`${e.name} status`} value={e.active ? "active" : "inactive"} onChange={(event) => void updateEmployeeStatus(e.id, event.target.value === "active")} className="rounded-full border border-violet-200 bg-violet-50 px-3 py-1 text-xs font-semibold text-violet-700"><option value="active">active</option><option value="inactive">inactive</option></select></article>) : visibleTimesheets.map(t => <article key={t.id} className="flex items-center justify-between border-b p-5 last:border-0"><div><p className="font-semibold">{t.employees?.name ?? "Employee"} · {t.hours}h</p><p className="text-sm text-slate-500">{t.work_date}{t.projects ? ` · ${t.projects.name}` : ""}</p></div><span className="rounded-full bg-violet-50 px-3 py-1 text-xs font-semibold text-violet-700">submitted</span></article>)}{(isTeam ? visibleEmployees : visibleTimesheets).length === 0 && <p className="p-6 text-sm text-slate-500">{isTeam && search ? "No employee matches your search." : "Nothing added yet."}</p>}</section>
  </AppShell>;
}
