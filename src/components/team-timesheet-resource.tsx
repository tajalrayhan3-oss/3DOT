...
export function TeamTimesheetResource({ type }: { type: "team" | "timesheets" }) {
  ...
  async function load() {
    ...
    const [{ data: staff }, { data: projectRows }, { data: timeRows }] = await Promise.all([
      supabase.from("employees").select("*").eq("company_id", company.id).order("created_at", { ascending: false }),
      supabase.from("projects").select("id,name").eq("company_id", company.id).order("name"),
    ...
    setEmployees(staff ?? []); setProjects(projectRows ?? []); setTimesheets((timeRows as Timesheet[] | null) ?? []);
  }
  useEffect(() => { load(); }, []);
