...
export function CompanyForm({ redirectTo = "/dashboard" }: { redirectTo?: string | null }) {
  ...
  async function loadCompany() {
    ...
    if (!user) return;
    const { data, error } = await supabase.from("companies").select("id,name,trn,city,email,phone,address,website,logo_url,letterhead_url,signature_url,stamp_url,document_footer").eq("owner_id", user.id).order("created_at", { ascending: false }).limit(1).maybeSingle();
  ...
  async function submit(formData: FormData) {
    ...
    const upload = async (field: "logo" | "letterhead" | "signature" | "stamp", currentUrl: string | null) => {
    ...
    try {
      const [logoUrl, letterheadUrl, signatureUrl, stampUrl] = await Promise.all([upload("logo", company?.logo_url ?? null), upload("letterhead", company?.letterhead_url ?? null), upload("signature", company?.signature_url ?? null), upload("stamp", company?.stamp_url ?? null)]);
      const values = { name: String(formData.get("name")), trn: String(formData.get("trn") || "") || null, city: String(formData.get("city")), email: String(formData.get("email") || "") || null, phone: String(formData.get("phone") || "") || null, address: String(formData.get("address") || "") || null, website: String(formData.get("website") || "") || null, logo_url: logoUrl, letterhead_url: letterheadUrl, signature_url: signatureUrl, stamp_url: stampUrl, document_footer: String(formData.get("document_footer") || "") || null };
    const { error } = company
