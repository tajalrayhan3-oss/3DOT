...
export function FinancialResource({ type }: { type: "quotations" | "invoices" }) {
  ...
  async function load() {
    ...
    const [clientsResult, recordsResult] = await Promise.all([
      ...
      supabase.from(type).select(quotation ? "id,client_id,number,title,amount,status,clients(name)" : "id,client_id,number,amount,status,due_date,clients(name)").eq("company_id", company.id).order("created_at", { ascending: false }),
    ]);
  ...
  async function createInvoiceFromQuotation(item: RecordItem) {
    ...
    const { error } = await supabase.from("invoices").insert({ company_id: companyId, client_id: item.client_id ?? null, number: `INV-${suffix}`, amount: item.amount, status: "draft", due_date: dueDate.toISOString().slice(0, 10) });
    setSaving(false);
    if (error) return setMessage(error.message);
    setMessage(`Invoice created from ${item.number}. Open Invoices to review it.`);
  }
