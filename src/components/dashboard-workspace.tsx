...
type DashboardData = { projects: Project[]; employeeCount: number; outstanding: number; clientCount: number };
const currency = (value: number) => `AED ${value.toLocaleString("en-AE", { maximumFractionDigits: 2 })}`;

export function DashboardWorkspace() {
  const [data, setData] = useState<DashboardData>({ projects: [], employeeCount: 0, outstanding: 0, clientCount: 0 });
  ...
  useEffect(() => { void (async () => {
    ...
    const [projectsResult, employeesResult, invoicesResult, clientsResult] = await Promise.all([
    ...
    const firstError = projectsResult.error || employeesResult.error || invoicesResult.error || clientsResult.error;
    if (firstError) return setMessage(firstError.message);
    const invoices = (invoicesResult.data ?? []) as Invoice[];
    setData({ projects: (projectsResult.data ?? []) as Project[], employeeCount: employeesResult.count ?? 0, outstanding: invoices.reduce((total, invoice) => total + Number(invoice.amount || 0), 0), clientCount: clientsResult.count ?? 0 });
  })(); }, []);
